defmodule RenewCollabWeb.HierarchyLayerComponent do
  ...
  def render(assigns) do
    ...
    <g style={"display: #{if(@layer.hidden, do: "none", else: "initial")}"} {if @selectable, do: ["phx-click": "select_layer"], else: []} phx-value-id={@layer.id}>
      <g {if(@selection==@layer.id and Enum.any?(@document.layers, &(&1.direct_parent && &1.direct_parent.ancestor_id == @layer.id)), do: [style: "outline: 3px solid pink;cursor:move;", "selected": true], else: [])} phx-hook="RnwGroupDragger" id={"layer-parent-group-#{@layer.id}"} rnw-layer-id={@layer.id} opacity={style_or_default(@layer, :opacity)}>
