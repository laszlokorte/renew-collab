          ...
          <ul style="margin: 0; padding: 0; list-style: none; display: flex; flex-direction: column; gap: 0.2ex">
            <%= for s <- snaps  do %>
              <li style="display: flex; align-items: center;gap: 1ex;">
                <%= if s.is_latest do %>
                  ...
                  <%= if not is_nil(s.label) do %>
                    <button
                      ...
                      phx-value-id={s.id}
                      type="button"
                      style="text-align: center; align-self: center; cursor: pointer; border: none; background: #eaa; color: #fff; width: 1.8em; height: 1.8em; display: grid; place-content: center; place-items: center; border-radius: 10%; font-weight: bold;"
                      title="Remove Pin"
                    >
                      📌
                    </button>
                  ...
                  <% else %>
                    <form
                      ...
                      rnw-snapshot-id={s.id}
                      id={"pin-snapshot-#{s.id}"}
                      style="display: flex; align-items: stretch; gap: 0.2ex"
                    >
                      <button
                        style="text-align: center; align-self: center; cursor: pointer; border: none; background: #aea; color: #fff; width: 1.8em; height: 1.8em; display: grid; place-content: center; place-items: center; border-radius: 10%; font-weight: bold;"
                        type="submit"
                      >
                        📌
                      </button>
                ...
                <% else %>
                  <button
                    style="font-size: 10pt; font-family: sans-serif; cursor: pointer; padding: 1ex; border: none; background: #333; color: #fff"
                    phx-click="restore"
                    phx-value-id={s.id}
                  >
                    Restore
