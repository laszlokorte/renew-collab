defmodule RenewCollabWeb.HierarchyLayerTextComponent do
  ...
  def render(assigns) do
    ...
    <g opacity={style_or_default(@layer, :opacity)} id={"#{@layer.text.id}-wrapper"}>
      <g
        data-background-box
        id={"#{@layer.text.id}-outline-box-container"}
      ...
      <text
        ...
        font-size={style_or_default(@layer.text, :font_size)}
      ...
      >
        ...
        -->
            <tspan
              ...
              {format}
              {if(li==0 and style_or_default(@layer.text, :rich), do: ["font-weight": "bold"], else: [])}
              x={@layer.text.position_x}
      ...
      <%= if @selected do %>
        <g
          cursor="move"
          phx-hook="RnwTextDragger"
          rnw-layer-id={@layer.id}
          id={"#{@layer.text.id}-outline-box-selection-dragger"}
        >
          <g data-background-box id={"#{@layer.text.id}-outline-box-selection-container"}>
            <rect
              ...
              fill-opacity="0.3"
              opacity="0.8"
              id={"#{@layer.text.id}-outline-box-selection"}
              phx-update="ignore"
              x={@layer.text.position_x}
              y={@layer.text.position_y}
              width="0"
