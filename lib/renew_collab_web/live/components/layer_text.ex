        ...
        <g opacity={style_or_default(@layer, :opacity)}  id={"#{@layer.text.id}-wrapper"}>
          ...
          <text 
            ...
            <%= for {{line, format}, li} <- @layer.text.body |> String.split("\n") |> filter_blank_lines(style_or_default(@layer.text, :blank_lines)) |> Enum.map(&format_line(style_or_default(@layer.text, :rich), &1)) |> Enum.with_index() do %>
              ...
              <tspan 
            {if(style_or_default(@layer.text, :underline), do: ["text-decoration": "underline"], else: [])} {format} {if(li==0 and style_or_default(@layer.text, :rich), do: ["font-weight": "bold"], else: [])} x={@layer.text.position_x} {[dy: if(li>0, do: "1.2em", else: "1em")]}><%= line|>String.trim() %></tspan>
              <% else %>
          ...
          <g cursor="move" phx-hook="RnwTextDragger" rnw-layer-id={@layer.id} id={"#{@layer.text.id}-outline-box-selection-dragger"}>
            ...
            <%= for {{line, format}, li} <- @layer.text.body |> String.split("\n") |> filter_blank_lines(style_or_default(@layer.text, :blank_lines)) |> Enum.map(&format_line(style_or_default(@layer.text, :rich), &1)) |> Enum.with_index() do %>
              <%= if String.trim(line) != "" do %>
              <tspan {if(style_or_default(@layer.text, :underline), do: ["text-decoration": "underline"], else: [])} {format} {if(li==0 and style_or_default(@layer.text, :rich), do: ["font-weight": "bold"], else: [])} x={@layer.text.position_x} {[dy: if(li>0, do: "1.2em", else: "1em")]}><%= line|>String.trim() %></tspan>
              <% else %>
